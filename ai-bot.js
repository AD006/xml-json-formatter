...
function detectContentType(content) {
  ...
  // Try to find JSON/XML inside text (for log files, embedded content)
  if (extractJSON(t)) return 'json';
  if (extractXML(t)) return 'xml';
  return 'unknown';
}

// Extract JSON from text by finding balanced braces
function extractJSON(str) {
  ...
  for (let i = start; i < str.length; i++) {
    ...
    if (char === '{') { braceCount++; }
    else if (char === '}') { braceCount--; if (braceCount === 0 && bracketCount === 0) return str.substring(start, i + 1); }
    else if (char === '[') { bracketCount++; }
    else if (char === ']') { bracketCount--; if (bracketCount === 0 && braceCount === 0) return str.substring(start, i + 1); }
  }
  return null;
}
...
function analyzeXML(content) {
  // Try to extract XML from text if content doesn't start with XML
  let xmlContent = content;
  if (!content.trim().startsWith('<') && !content.trim().startsWith('<?xml')) {
    const extracted = extractXML(content);
    if (extracted) xmlContent = extracted;
  }

  const tags       = [...xmlContent.matchAll(/<([a-zA-Z][a-zA-Z0-9_:.-]*)[\s>\/]/g)].map(m => m[1]);
  const unique     = [...new Set(tags)];
  const attrs      = [...xmlContent.matchAll(/([a-zA-Z][a-zA-Z0-9_:]*)=["']([^"']*)["']/g)].map(m => ({ name: m[1], value: m[2] }));
  const textNodes  = [...xmlContent.matchAll(/>([^<\n]{1,80})</g)].map(m => m[1].trim()).filter(Boolean);
  const rootMatch  = xmlContent.match(/<([a-zA-Z][a-zA-Z0-9_:.-]*)[\s>]/);
  const lineCount  = content.split('\n').length;
  const xmlDecl    = xmlContent.includes('<?xml');
...
function analyzeJSON(content) {
  ...
  try {
    ...
    try {
      ...
      // Try extracting JSON from text (for embedded JSON in log files)
...
function recognize(text) {
  ...
  // Structure questions - check these BEFORE find to avoid "list keys" being caught as find
  ...
  // Find/Search - more natural (check after structure questions)
  ...
  if (/summarize|analyz|tell me about|what is this|explain (this|the)|describe|understanding|what does|what's in|check this/i.test(t)) return 'summary';
